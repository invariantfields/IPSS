"""Iterative Product State Splitting (IPSS) operation."""
...
def Max_overlap_product_state(
    rho: np.ndarray, dim: int | list[int] | None = None, Init_guess: np.ndarray | None = None, search_depth: int = 2
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    r"""Maximum overlap product state algorithm.
    ...
    Raises:
        ValueError: If the input `rho` is not a valid quantum state.
        ValueError: If the input `dim` is not a valid input.i.e.,`dim`is not a list of two integers or a single integer
        ValueError: If the input `Init_guess` is not a valid initial guess.
        ValueError: If the input `Init_guess` is not a product state.
        ValueError: If the input `search_depth` is not a valid search depth.

    Args:
        rho: A bipartite quantum state to compute the Schmidt decomposition of.
        dim: An array consisting of the dimensions of the subsystems (optional, default gives subsystems
                equal dimensions).
        Init_guess: A product state to start the iterative process from(optional).
        search_depth: The number of iterations to perform (default is 2).
    ...
    # Allow the user to enter a single number for `dim`.
        ...
        raise ValueError("Invalid input for `dim`. `dim` must be a list of two integers or a single integer.")
